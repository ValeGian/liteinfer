...
## 24-09-2026 — §2.10 A prompt chunk reads its prefix where it lies

- **PRs.** [#44](https://github.com/ValeGian/liteinfer/pull/44)
- **What.** `paged_decode` read one query per sequence, so a chunk continuing a cached prompt could not use it: §1.7 served those chunks by copying every token the sequence held back out of the pool and handing the copy to FlashAttention's varlen entry. `paged_prefill` gives the same kernel a query dimension — `query_start_loc` beside `context_lens`, one program per block of 16 queries x the query-head group, the cached prefix folded without a mask and only the block's own span causally — and the packed chunk payload now returns a `PagedKV`. Nothing is copied. `VarlenKV` is back to one set of boundaries, since flash no longer sees a sequence with fewer queries than keys.
- **Decode is the one-query case, and did not move.** 162 of 162 shapes bit-identical to the previous kernel — batch 1-32, query-head groups 1-8, head dims 64-128, fp32 and bf16, split and unsplit. It stays its own launch over the shared body, because sharing the chunk kernel's signature cost decode 5 us of host time per layer per eager step (Triton's launcher handles each argument in Python); as its own kernel it is 47.1 against 47.5 us per call.
- **Measured per layer** at Llama-3.2-1B's shapes on an A40 (`docs/benchmarks.md`, §2.10): **0.55x** to 0.95x of copy-then-flash across short chunks and short prefixes, **0.17x** for decode rows beside a chunk — the batch §1.3 will create, where the copy reads out every row's whole context — and **1.03x to 1.12x** for large chunks over long prefixes, where the work is dot products and FlashAttention's MMA loop is faster. Folding the prefix unmasked took that worst case from 1.17x to 1.13x, a 32-key tile to 1.12x; warps and pipeline depth did not move it. No default run chunks, so no engine number moves.
- **Shared memory decides what compiles, and it differs per GPU.** A chunk's 64-row query tile against a 128-wide head overflows an A40 in fp32 at Triton's default pipeline depth (115 KiB of 99), and a 256-wide one fits only at 16 keys and one stage. `paged_prefill` searches on a shape's first launch — the requested key tile at each depth, then half the tile — and caches what fitted. The answer does not depend on either knob.
- **Parity, and a tolerance that was below the precision it checked.** The kernel is pinned against `eager` in fp32 at 1e-5 over mixed one-query and many-query batches, every tile size, grouped and ungrouped heads, and head dims that are not powers of two. At engine level, §1.7's chunked-against-whole logit tests failed at their 2^-5 tolerance: the logits reach ~60, where a bf16 ulp is 0.25, and the whole run now goes through a different kernel than the chunked one. They now measure both runs against a single-precision reference and require the chunked run's drift to be at most twice the whole run's — 0.10-0.14 for both, against 3.19 for a chunk ignoring its prefix and 0.39 for a context one token short, both planted to check.
- **For §1.3.** Over whole prompts the paged kernel is **0.30x** of FlashAttention at 32 x 18 tokens and within 4% up to 4,096, so one kernel for every row is affordable, and `varlen_attention` can go when an engine run agrees. §2.11 is filed for the one shape the default tiles leave on the table: short chunks over long prefixes want 8 x 128.

---

## 23-09-2026 — §1.7 + §2.9 The scheduler hands out tokens, not slots

- **PRs.** [#43](https://github.com/ValeGian/liteinfer/pull/43)
- **What.** `ContinuousScheduler` admitted whole sequences until `max_num_seqs` slots were full, so a 10,000-token prompt and a 17-token one cost the same slot and a step's work was whatever they happened to add up to. Each `Sequence` now carries `num_computed_tokens`, and every step grants it what it is owed — the rest of its prompt, or one sampled token — clamped by a token budget, `EngineConfig.max_num_batched_tokens`. Two caps, both binding. A prompt larger than what is left of the budget is **chunked** across steps rather than deferred. `ContinuousSchedulerOutput` is now `num_scheduled_tokens` per request; the `prefill_seqs` / `decode_seqs` split survives only in the engine's `_by_phase`, which is the one thing §1.3 has left to remove.
- **Kept to the first twenty lines of vLLM's loop.** Running sequences first, in admission order, then waiting ones into free slots while budget is left. A chunked prompt takes everything that remains, so nothing is admitted behind it — at most one sequence is ever part-way through its prompt, and it is always the last running. With the budget held at `>= max_num_seqs` (enforced, as vLLM does), the decodes ahead of it can never starve it.
- **A chunk had to attend to its own prefix, and no kernel could.** The item was filed as scheduling, but the second chunk of a prompt needs attention over K/V an earlier pass wrote — which is §2.10's problem. It is served here by a copy §2.10 will remove: the payload writes the chunk, then reads the sequence's whole context back from the pool. Packed, that goes to FlashAttention's varlen entry with separate `cu_seqlens_k`; PyTorch's private op aligns its causal diagonal to the bottom right when a sequence has fewer queries than keys, which its signature does not state, so a test now pins it against `eager`. Padded, `build_prefill_mask` takes `context_lens` and becomes one offset diagonal for the whole batch — and the decode mask turned out to be its one-query case, so it is now built by the same rule instead of a copy of it.
- **§2.9 rode along, as filed.** Decode wrote through `slot_table[:, -1:]`, which is the newest token only because the table is right-aligned. Both decode payloads, and the captured graphs' static buffers, now take a flat mapping — `slot_mapping` over a batch where every count is 1 — and a test pins it byte for byte against the old column. The right-aligned table remains for decode *reads* and the padded prefill's writes, which is what §3.9 finishes. `slot_mapping` and `slot_table` both take window `starts` now, and every cache address is "each sequence's newest `counts` tokens", so a whole prompt, a chunk and a decode step are one request.
- **What doubting the cost found.** Built the obvious way, the decode write mapping went through the device path — about ten launches and three blocking transfers — and measured **0.28 ms at batch 1**, ~5% of a captured decode step, for an item that was supposed to move nothing. Single-token windows are now answered on the host in one transfer: **0.021 ms** at batch 1, 0.048 ms at 128. The window also stopped the device path reading whole block tables when it needs one block: the read table alone costs 3.4 ms to build at 128 sequences of 4,000 tokens, and paying that twice per step was the first draft. What remains, write plus read against master's read alone: **+0.045 ms per step at batch 1** (~0.8% of a captured step, inside run-to-run variance) and −0.4 ms at 128, where the write no longer rides on the full table.
- **No default moves, with one exception.** `max_num_batched_tokens=None` resolves to `max_num_seqs x max_model_len`, which no step can exceed, so nothing is chunked unless a config asks — admission, passes and outputs are what they were, and no stored result is invalidated. The exception is a prompt that encodes to no tokens: master generated from it (64 `<unk>` on the tiny model, from nothing), here it would owe the scheduler zero tokens and trip an engine invariant that takes every request down, so it is refused at admission and fails only its own caller. The activation profile stays a safe bound under a budget: no chunked pass is wider than the `max_num_seqs x max_model_len` prefill it measures, and the one thing a continuing chunk adds — its context read back out of the pool — is one layer's K/V at a time, against a profile whose mask alone is 1 GiB at 32 x 4,096. Measured against master (`19abdf2`), back to back on one A40, same datasets, 200 samples: ITL p50 6.582 → 6.662 ms at batch 1 (1.012x, consistent across p50/p95/p99, so probably real — about what the decode addressing and the scheduler's per-step bookkeeping predict), TTFT p50 15.65 → 15.92 ms, throughput 3,208.2 → 3,245.7 tok/s on mixed ≤2048 / OSL 128 and 3,342.4 → 3,292.2 on ISL 128 / OSL 256 — all inside run-to-run variance. No budgeted row: the harness records no per-token timing under concurrent load, the only place chunking pays, so a row would price its cost and not its benefit.
- **Parity is on logits, because greedy tokens proved blind.** The first parity tests compared greedy output, budgeted against unbudgeted — and survived every bug planted to check them: an off-by-one in the chunk mask, a padded write ignoring its window offset, a packed chunk ignoring its prefix. The tiny random model mostly repeats its input token whatever attention returns. So chunked prefill is pinned by logits instead: on CPU in single precision through the padded path, and on CUDA through the packed path, a prompt prefilled in chunks that straddle block boundaries predicts what one pass predicts, and the decode steps after it read the same history. Each planted bug fails them. The greedy engine tests stay, described as what they are — admission mid-generation, chunking and delivery end to end.
- **The same blindness was already in `test_cuda_graphs_gpu.py`.** Replays that never refilled the decode write buffer — every step's K/V into the null block — passed all ten existing graph tests, which compare greedy tokens too. That predates this change but is the path it rewrote, so a captured-against-eager logits test now covers it, and fails on that bug.
...
- **What.** §3.2 left the decode step nearly flat in batch width — the same weights are read once per step however many sequences share it — so every extra sequence is close to free. `max_num_seqs=128` measures **6,945.3 tok/s against 3,152.4 at 32, 2.20x**, with wall time 16.2 → 7.4 s.
- **And it does not close the gap to vLLM; it widens it.** At matched width vLLM goes 4,466.6 → 11,097.5, which is 2.48x over the same widening, so the ratio moves **0.71x → 0.63x**. That is the finding worth having, and it is invisible if a wide engine is only compared to its own narrower self — which is why `liteinfer-graphs-b128` carries no `baseline` and `vllm-b128` was added beside it. The benchmark matrix gained a `max_model_len` knob so a row can state the context budget it claims.
...
- **PRs.** [#36](https://github.com/ValeGian/liteinfer/pull/36)
- **What.** Two changes that turned out to be one: sizing the KV pool from a *measured* activation budget was blocked by a prefill allocation that should never have existed.
- **§3.7 — the head ran everywhere and one row was read.** `LlamaForCausalLM.forward` applied the LM head to every position and `prefill` returned `logits[:, -1, :]`. At batch 8 with 2,048-token prompts that tensor was **3.91 GiB of a 4.05 GiB prefill peak — 96% of it — and 8 of its 16,384 rows were read**. `forward` now takes a `logits_positions` slice; `None` still computes them all, which is the path the `transformers` parity tests compare, and every caller in the engine passes `LAST_POSITION`. Prefill peak **3.98 → 1.01 GiB**, logits bit-identical.
- **Measured, and it is a capability fix rather than a throughput one.** TTFT p50 at ISL 3584 goes **162.5 → 147.2 ms (1.10x)** and throughput at ISL 2048 / OSL 128 **908.2 → 979.0 (1.08x)**; the other four shapes move 1.00-1.04x, because prefill is a smaller share of them. Nothing regressed. The earlier note that this would be "a throughput win at prefill-heavy shapes" was optimistic by about half — the arithmetic said the head is a fifth of prefill's flops, and prefill is only part of a run.
- **§2.6 — the activation budget is now a number this device measured.** The pool took a fraction of *free* VRAM, so its size depended on what had been resident a moment earlier: torch's caching allocator keeps freed blocks, and the pool lost **19% of them (72,516 → 58,589) to 8 GiB that no longer existed**. `gpu_memory_utilization` replaces `kv_cache_memory_fraction`, applies to total memory less the resident weights — vLLM's meaning for the same name — and then subtracts a profiled figure: one prefill at `max_num_seqs x max_model_len`, which is not hypothetical because `schedule()` admits and prefills that many at once. On Llama-3.2-1B at 32 x 4,096 it is **9.04 GiB**.
- **Which §3.7 is what made possible.** The same profile with the head unsliced needs **32.82 GiB**, and a 44 GiB card has nowhere to put weights and a pool beside it. The roadmap had §2.6's second step blocked on "bounding the worst case" without knowing what was making it large; it was this.
- **The profile needs no pool, which is the trick.** `ProfilePayload` returns the K/V the pass just computed, exactly as the prefill payload does after storing it, so the forward has the same shapes and the same peak while writing nowhere. That is what lets the measurement happen *before* the thing it is sizing exists.
- **A test caught the first forward measuring itself.** Unwarmed, a 2-sequence config profiled **8.49 MiB where a 32-sequence one profiled 5.77** — the first forward in a process allocates cuBLAS workspaces that every later one reuses, and they landed inside the measurement, so the first engine loaded in any process would have been handed the smallest pool. One throwaway 16-token pass fixes it; the figures then scale exactly 2x per doubling of the batch.
- **And it is a behaviour change where the old number was being clipped.** Loading weights leaves enough allocator residue that free reads well below `total - weights`: at Llama-3-8B's defaults the pool came out **5,752 blocks (11.24 GiB) where the config asks for 8,192 (16.00 GiB)**. The engine used to take less than it was told whenever loading was untidy, and now takes what it was told. `tests/e2e/test_llama3_8b_gpu.py` holds a second copy of the same 8B model as its reference and had been fitting on that accident; it now asks for the one short sequence it decodes.
- **What it unblocks.** §1.4 was gated on both of these and is down to one prerequisite, the capture ladder. Raising concurrency now demands memory honestly rather than discovering the shortfall under load, and the 62.6 GiB logits tensor that 128 sequences of 2,048 tokens would have built is gone.
...
- **What.** A decode step issued about 700 kernels to do 7 ms of GPU work inside a 13 ms step. `EngineConfig.enable_cuda_graphs` now captures that forward once per batch width and replays it, which submits the whole sequence in one go instead of paying host dispatch per kernel. Decode step **13.3 → 6.6 ms**, throughput **1,932 → 3,152 tok/s**.
- **Measured, against the same engine with capture pinned off.**

  | throughput | before | after | |
  |---|---:|---:|---:|
  | 128 / 256 | 1,931.5 | **3,152.4** | **1.63x** |
  | 128 / 1024 | 1,930.6 | 2,972.1 | 1.54x |
  | 1024 / 1024 | 1,817.4 | 2,389.2 | 1.31x |
  | 1024 / 256 | 1,569.6 | 2,071.6 | 1.32x |
  | 2048 / 128 | 892.6 | 979.5 | 1.10x |

  Latency at B=1: ITL p50 13.3 → **6.6 ms** (2.01x), e2e 3,407.6 → **1,698.5 ms**, and 16.9 → 7.9 ms at 3,584 tokens of context, where the spread is wide enough (about 5% run to run) that the honest reading is 2.0-2.4x. The gap to vLLM closes from 0.43x to **0.71x** on throughput and from 2.56x to **1.27x** on the step. Largest single change the project has measured, and general: every batch width, every context, both modes.
...
- **It is vLLM's `FULL_DECODE_ONLY`, and that is a scoping decision rather than an oversight.** A graph can hold the whole forward only where the batch is uniform, which a decode batch is and a prefill or mixed batch is not. vLLM covers the rest with **piecewise** capture — the inductor graph split at the attention ops, so attention runs outside and everything else is captured for any shape — and defaults to both together. liteinfer has no compilation step to split, so the full/decode-only half is the reachable one; the other half is filed as §3.8 with its own measurements, and it is worth little here until §1.3 exists, because an unchunked prefill is GPU-bound (37% idle at 128 tokens, 6% at 512, 1% at 2,048) rather than launch-bound. §1.3 is also what would take some of this milestone back: a mixed pass is not a uniform decode batch, so it would not be captured.
...
- **Measured, both kernels in one session, same datasets.**
  ...
  | shape | `sdpa` | `paged` | |
  |---|---:|---:|---:|
  | 128 / 256 | 1,748.1 | 1,931.5 | 1.10x |
  | 128 / 1024 | 1,232.2 | 1,930.6 | 1.57x |
  | 1024 / 256 | 793.1 | 1,569.6 | 1.98x |
  | 1024 / 1024 | 696.3 | 1,817.4 | **2.61x** |
  | 2048 / 128 | 418.8 | 892.6 | 2.13x |
...
- **At B=1 it buys nothing.** Latency mode, 200 requests one at a time: ITL p50 13.9 → **13.3 ms**, E2E 3,561.0 → **3,407.6 ms**, TTFT 13.9 → 13.9 ms. Every one of those is inside the ±4% floor, so the result is *no effect*, and it is the same result as the 1.12x row read differently. A single request has ~190 tokens of history to gather, which is the least this change can save, and the kernel's grid is one program per (sequence, KV head) — 8 programs on an 84-SM GPU. Per call it is 2.27x at B=1 against 7.73x at B=32. Nothing regressed; the single-request case is simply where this design is weakest, and flash-decoding's split-K is filed as §2.7.
